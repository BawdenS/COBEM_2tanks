...
% Planejamento de Trajetória - Saída Plana (Z)
  ...
  % Trajetória Constante com 2 valores (STEP)
    fP1 = 0.30; fP2 = 0.60;
    Zd = 0.50 + 0*t; Zd((round(fP1*st)):end) = 0.8; Zd((round(fP2*st)):end) = 0.20;
    d1Zd = 0*t; d2Zd = 0*t;
...
result_PSO(count) = global_best_fitness;
kP_f(count) = abs(current_position(1,i));
kI_f(count) = abs(current_position(2,i));
kD_f(count) = abs(current_position(3,i));
[melhor_fit,pos_fit] = min(result_PSO);
variavel_custo_temp=0;
...
display(['O melhor fitness encontrado da função objetivo foi: ', num2str(melhor_fit)]);
display(['Os melhores ganhos PID: Kp; Ki e Kd são: ', num2str(kP_f(pos_fit)),';', num2str(kI_f(pos_fit)),';',num2str(kD_f(pos_fit))])
kP = kP_f(pos_fit);kI = kI_f(pos_fit);kD = kD_f(pos_fit);
sim('TwoTanks_PID_R2018a'); plotResultados_NL;
save('PSO_workspace_PID_v1.0_agoravai_refnova.mat')

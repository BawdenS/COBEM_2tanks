...
%           p1 = 0.75; P1 = poly(-[p1 p1]); K1 = P1(1,2); K0 = P1(1,3);
        ...
        for kii=1:length(t-1) %SIMULAÇÃO DEVE CONERGIR EM ATÉ 15s
            ...
            variavel_custo_temp = variavel_custo_temp+ ((3*(abs(h2f(kii)-h2d(kii)))+0*(abs(h1f(kii)-h1d(kii)))).^2)*t(kii); % func obj de acordo com artigo que oniram mandou
        end

%         for ki = 1:1:(size(ff,1)-1)
%             variavel_custo_temp = variavel_custo_temp + konstante*abs(ff(ki) -  ff(ki+1));  
%         end

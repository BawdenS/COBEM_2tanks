...
nome_g_ideal = 'cobem_g_ideal_v2'; % nome arquivo simulink para comparar
...
for i = passo_tempo+1:passo_tempo:size(Vetor_obj_H1,1)
   if abs(mean(Vetor_obj_H1(i-passo_tempo:i) - Vetor_obj_H1(i-passo_tempo+1:i+1))) < 0.00005
       tempo_acm_h1 = saida_ideal{1}.Values.Time(i); % pega tempo de acomodação
   break;
   end
end
...
%   Kp Ki Kd                                       
ub=[200 100 20]'; %/*upper bounds of the parameters. */
lb=[0.0001 0 0]';%/*lower bound of the parameters.*/  
...
Ki = sol(2);
Kd = sol(3);
sim('COBEM_SIMULINK_OFICIAL'); plotResultados_NL;
% 
% plot_simu = sim(simIn);
% tempo_simu = 50;
